  ...
  <div class="orderCreate_container">
    ...
    >
      <div class="navbar1">
        ...
        <van-button class="follow-newCustomer-btn" @click="newCustomer">新建客户</van-button>
      </div>
      <van-search
        v-model="followVal"
...
<!--以下为新建客户弹出框    -->
    ...
    >
      ...
      <van-form>
        ...
        <van-field
          ...
          readonly
          clickable
          v-if="addList.customerStatus == this.columns1[0]"
          @click="toAddFollow"
        />
        <!-- 提交按钮 -->
        <div style="margin: 16px">
    ...
    <van-popup
      ...
      duration="0"
    >
      <van-button class="follow-cancel-btn" @click="folCancel1">取消</van-button>
      <van-search
        v-model="followVal1"
        placeholder="请输入搜索关键词"
        @search="onFollowSearch1"
        @cancel="onFollowSearchCancel1"
      />
      <van-cell
        v-for="item in followList"
        :key="item.id"
        @click="followConfirm1(item)"
      >
        <!-- 跟进人-跟进人信息 -->
        <van-row>
          <!-- 跟进人-跟进人头像 -->
...
export default {
  ...
  methods: {
    ...
    chooseBuyer(){
      ...
      this.onLoad();
      // let url = "/api/se/customer/query";
      // // let url = JSON.parse(Url).contextShare.queryList;
      // let postData = {
      //   currentPage: this.pageProps.pageNum++,
      //   pageCount: this.pageProps.pageSize,
      // }
